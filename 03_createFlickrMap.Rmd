...
```{r plot maps, include=FALSE}
# helper mapping 
bk_mapTheme <- function(
  base_size = 10.5, base_family = "OpenSans-CondensedLight",
  title_family = "OpenSans-CondensedBold", subtitle_family = "OpenSans-CondensedLightItalic",
  bg.colour = '#1a0000', colour = colourText_bkbg
    ...
    theme(
      ...
      legend.text = element_text(colour = "#f2f2f2", size = 7),
      legend.title = element_text(colour = "#f2f2f2", size = 9),
...
map1 <- ggplot(co.df) + 
  ...
  geom_polygon(
    ...
    size = 0.25, alpha = 0, color = "#404040") + 
  geom_polygon(
    data = lakes.df,  aes(x = long, y = lat, group = group),
    size = 0, fill = "#404040", colour = "#404040") +
  geom_jitter(
    data = data, aes(x = lon, y = lat, group = 1), 
    colour = "#cceeff", alpha = 0.2, size = 1/10^20) + 
...
map2 <- ggplot(co.df) + 
  ...
  labs(title = txt['title2', lang], subtitle = txt['descr2', lang])
...
save_html(
  tags$html(
    ...
    tags$body(
      div(class="graphic",
          img(src = basename(map1.path), width= "100%"),
          img(src = basename(map2.path), width= "100%")      
      ),
...
mapTopMuni <- function(dd, title, footer) {
  ...
  mapt <- ggplot(co.df) + 
    ...
    geom_polygon(
      aes(x = long, y = lat, group = group), 
      size = 0.5, alpha = 0, color = "#d9d9d9") +
    geom_polygon(
      data = ca.df, 
      aes(x = long, y = lat, group = group), 
      size = 0.1, alpha = 0, color = "#595959") +  
    geom_polygon(
      data = lakes.df,  aes(x = long, y = lat, group = group),
      size = 0, fill = "#595959", colour = "#595959") + 
    geom_point(
      data = dd, aes(x = lon, y = lat, group = 1, size = var), 
      colour = "#f5f0f0", alpha = 0.8) + 
    geom_label(
      data = dd, aes(x = lon, y = lat, group = 1, label = label),
      fill = "#666666",
      family = "OpenSans-CondensedLight",
      colour = colourText_bkbg,
      size = 3,
    ...
    labs(title = title, footer = footer)
